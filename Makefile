...
ESP_ROOT ?= $(HOME)/workspace/Arduino
# Output directory
BUILD_ROOT ?= ./build
LOG ?= build/USB0.LOG
DEFINE ?= ANCHOR
...
$(MAIN_EXE): $(CORE_LIB) $(USER_OBJ)
	...
	$(LD) $(LD_FLAGS) -Wl,--start-group $^  $(LD_STD_LIBS) -Wl,--end-group -L$(OBJ_DIR) -o $(MAIN_ELF)
...
flash: $(MAIN_EXE)
	$(RESET) $(UPLOAD_PORT)
	$(ESPTOOL) --port $(UPLOAD_PORT) -b $(UPLOAD_SPEED) read_mac
...
#	tools/esptool2 -debug -bin -boot2 -1024 -dio -40  $(MAIN_ELF) $(MAIN_ELF).bin .text .data .rodata # was 4096
	$(ESPTOOL) --p $(UPLOAD_PORT) -b $(UPLOAD_SPEED) write_flash  -ff 40m -fm dio -fs 32m \
		0x00000 $(MAIN_EXE) 
